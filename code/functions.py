...
def regroup_by_age(
    ...
    out = np.random.randint(
        low=0, high=15, size=((len(toAgeSplits) - 1,) + inp.shape[1:])
    )
...
agePopulationTotal = 1000.0 * np.array(
    [
        8044.056,
        7642.473,
        8558.707,
...
ageSocialMixingBaseline = (
    pd.read_csv(
        "data/socialcontactdata_UK_Mossong2008_social_contact_matrix.csv", sep=","
    )
    .iloc[:, 1:]
    .values
)
ageSocialMixingDistancing = (
    pd.read_csv(
        "data/socialcontactdata_UK_Mossong2008_social_contact_matrix_with_distancing.csv",
        sep=",",
    )
    .iloc[:, 1:]
    .values
...
withinHospitalSocialMixing = elevatedMixingRatioInHospital * np.sum(
    np.dot(agePopulationRatio, ageSocialMixingBaseline)
)

ageHospitalisationRateBaseline = (
    pd.read_csv(
        "data/clean_hosp-epis-stat-admi-summ-rep-2015-16-rep_table_6.csv", sep=","
    )
    .iloc[:, -1]
    .values
)
ageHospitalisationRecoveryRateBaseline = (
    1.0
    / pd.read_csv("data/clean_10641_LoS_age_provider_suppressed.csv", sep=",")
    .iloc[:, -1]
    .values
)

initBaselineHospitalOccupancyEquilibriumAgeRatio = ageHospitalisationRateBaseline / (
    ageHospitalisationRateBaseline + ageHospitalisationRecoveryRateBaseline
)

ageNhsClinicalStaffPopulationRatio = (
    pd.read_csv("data/clean_nhsclinicalstaff.csv", sep=",").iloc[:, -1].values
)

infToHospitalExtra = np.array([1e-4, 1e-3, 2e-2, 1e-2])

relativeAdmissionRisk_given_COVID_by_age = np.array(
    [
        -0.94886625,
        -0.96332087,
        -0.86528671,
...
riskOfAEAttandance_by_age = np.array(
    [
        0.41261361,
        0.31560648,
        0.3843979,
...
relativeDeathRisk_given_COVID_by_age = np.array(
    [
        -0.99742186,
        -0.99728639,
        -0.98158438,
...
ageRelativeRecoverySpeed = np.array(
    [0.0] * 9
)  # For now we make it same for everyone, makes calculations easier

caseFatalityRatioHospital_given_COVID_by_age = np.array(
    [
        0.00856164,
        0.03768844,
        0.02321319,
...
df_CHESS = pd.read_csv(
    "data/fake_CHESS.csv", sep=",", converters={"DateOfAdmission": lambda x: str(x)}
)
...
df_CHESS = (
    df_CHESS.sort_index()
    .drop("DateOfAdmission", axis=1)
    .query("20200309 <= index <= " + CONST_DATA_CUTOFF_DATE)
)

# Get number of tests per age group
df_CHESS_numTests = df_CHESS.loc[
    :, df_CHESS.columns.str.startswith("AllAdmittedPatientsTestedForCOVID19")
]

# Change age groups to reflect our groupings
df_CHESS_numTests_regroup = pd.DataFrame(
    data=regroup_by_age(
        inp=df_CHESS_numTests.to_numpy().T,
        fromAgeSplits=np.concatenate(
            [np.array([1, 5, 15, 25]), np.arange(45, 85 + 1, 10)]
        ),
...
def adjustRatesByAge_KeepAverageRate(
    rate, ageRelativeAdjustment, agePopulationRatio=agePopulationRatio, maxOutRate=10
):
    ...
    while np.sum(out >= maxOutRate) > 0:
        ...
        if np.sum(out >= maxOutRate) > 0:
            warnings.warn(
                "covidTesting::adjustRatesByAge_KeepAverageRate Adjusted rate larger than {} encountered, reducing ageAdjustment variance by 10%".format(
                    maxOutRate
                )
            )
            # print(out)
...
# OLD regroup by age....not used since we don't have access to the CHESS data
"""
